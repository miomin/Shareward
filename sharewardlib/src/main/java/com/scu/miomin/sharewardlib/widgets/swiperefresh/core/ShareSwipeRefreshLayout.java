package com.scu.miomin.sharewardlib.widgets.swiperefresh.core;
 ...
 */
public class ShareSwipeRefreshLayout extends FrameLayout implements NestedScrollingParent {
    ...
    private ShareGuidanceView headerView = new ShareGuidanceView(getContext());
    private ShareGuidanceView footerView = new ShareGuidanceView(getContext());
    ...
    public ShareSwipeRefreshLayout(Context context) {
        super(context);
        initAttrs(context, null);
    }

    public ShareSwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttrs(context, attrs);
    }

    public ShareSwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttrs(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public ShareSwipeRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        int resId;
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ShareSwipeRefreshLayout);
        Resources resources = context.getResources();

        //Indicator背景颜色
        resId = ta.getResourceId(R.styleable.ShareSwipeRefreshLayout_refresh_view_bg_color, -1);
        if (resId == -1) {
            mGuidanceViewBgColor = ta.getColor(R.styleable.ShareSwipeRefreshLayout_refresh_view_bg_color,
                    Color.WHITE);
        } else {
        ...
        //加载文字颜色
        resId = ta.getResourceId(R.styleable.ShareSwipeRefreshLayout_refresh_text_color, -1);
        if (resId == -1) {
            mGuidanceViewTextColor = ta.getColor(R.styleable.ShareSwipeRefreshLayout_refresh_text_color,
                    Color.BLACK);
        } else {
        ...
        //进度条背景颜色
        resId = ta.getResourceId(R.styleable.ShareSwipeRefreshLayout_progress_bg_color, -1);
        if (resId == -1) {
            mProgressBgColor = ta.getColor(R.styleable.ShareSwipeRefreshLayout_progress_bg_color,
                    Color.WHITE);
        } else {
        ...
        //进度条颜色
        resId = ta.getResourceId(R.styleable.ShareSwipeRefreshLayout_progress_bar_color, -1);
        if (resId == -1) {
            mProgressColor = ta.getColor(R.styleable.ShareSwipeRefreshLayout_progress_bar_color,
                    Color.RED);
        } else {
        ...
        //下拉刷新文字描述
        resId = ta.getResourceId(R.styleable.ShareSwipeRefreshLayout_refresh_text, -1);
        if (resId == -1) {
            mRefreshDefaulText = ta.getString(R.styleable.ShareSwipeRefreshLayout_refresh_text);
        } else {
            mRefreshDefaulText = resources.getString(resId);
        }

        //上拉加载文字描述
        resId = ta.getResourceId(R.styleable.ShareSwipeRefreshLayout_load_text, -1);
        if (resId == -1) {
            mLoadDefaulText = ta.getString(R.styleable.ShareSwipeRefreshLayout_load_text);
        } else {
